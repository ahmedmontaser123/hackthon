...
final_df = studentAssessment.merge(assessments, how='left', on='id_assessment')

final_df["score"] = (final_df["score"]/100) * final_df["weight"]
exams = final_df[final_df['assessment_type'] == 'Exam']
assignments = final_df[final_df['assessment_type'] != 'Exam']


assignments = assignments.groupby(['code_module','code_presentation','id_student'])['score'].sum()
exams = exams.groupby(['code_module','code_presentation','id_student'])['score'].sum()

merged = pd.merge(assignments, exams , how='left', on=['code_module','code_presentation','id_student'])
...
final_df = final_df.merge(studentInfo, how="left", on=["code_module", "code_presentation","id_student"])
...
final_df = final_df.rename({"score_x":'assignment_score',"score_y":"exam_score"},axis=1)
